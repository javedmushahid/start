...
const forgotPassword = async (req, res) => {
  ...
  const resetPassword = async (req, res) => {
    try {
      ...
      const confirmHashedPassword = hashPassword(confirmPassword, process.env.SALT || "");
...
const getAllUsers=async(req,res)=>{
  ...
  try {
    ...
    console.error(error);
    res.status(INTERNAL_SERVER_ERROR).json({
      error: "An error occurred while fetching all users data.",
    });
  }
}

module.exports = { signUp, login, signout ,forgotPassword,resetPassword,validateOTP,getAllUsers};
